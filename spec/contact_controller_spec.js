...
describe("ContactController", () => {
    ...
    describe('#addContact()', ()=>{
        it('should add a single contact into the book', (done)=>{
            this.book.addContact('Alice', '001-101-1010', 'alice@google.com')
            .then((contact)=>{
                expect(contact.name).toBe('Alice');
                expect(contact.phone).toBe('001-101-1010');
                expect(contact.email).toBe('alice@google.com');
                done();
            })
            .catch((err)=>{
                done();
                    ...
                    this.book.addContact(...zelda).then(()=>{
                        this.book.addContact(...snake).then(()=>{
                            this.book.getContacts().then((contacts)=>{
                                let contact = this.book.binarySearch(sort(contact), 'Magus Johnson');
                                expect(contact.name).toBe('Magus Johnson');
                                expect(contact.phone).toBe('101-010-101');
                                expect(contact.email).toBe('magus@squaresoft.com');
                                done();
